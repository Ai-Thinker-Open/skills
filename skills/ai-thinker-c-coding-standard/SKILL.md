...
所有函数（包括 `.h` 声明和 `.c` 定义）都必须添加 Doxygen 风格的函数头注释。

**强制要求：所有注释必须使用中文，禁止使用英文注释。**
...
要点：
- **所有注释字段（@brief、@param、@return、@note 等）必须使用中文**
- 仅 `@brief` 必填；无参数用 `void` 作入参
- 多个返回值/错误码时，在 `@return` 下方用 `- 错误码: 描述` 格式逐行列出，缩进对齐
- `@return` 行格式：`@return 返回值类型 返回值说明`
...
- **所有代码注释强制使用中文，包括但不限于：函数头注释、行注释、文件说明注释。禁止使用英文注释。**
